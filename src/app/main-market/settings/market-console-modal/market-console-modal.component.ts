...
import { MarketRpcService } from 'app/main-market/services/market-rpc/market-rpc.service';
...
@Component({
  templateUrl: './market-console-modal.component.html',
  styleUrls: ['./market-console-modal.component.scss']
})
export class MarketConsoleModalComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  @ViewChild('debug', {static: true}) private commandContainer: ElementRef;
  ...
  rpcCall() {
    ...
    const commandString = String(params.shift());
    const callableParams: (string|number|boolean|null)[] = params;
  ...
  private formatErrorResponse(error: any) {
    if (error.code === -1) {
      ...
      const errorMessage = errorStr.includes('unknown command') ||
        errorStr.includes('unknown subcommand') ||
        errorStr.includes('command not found') ?
        TextContent.INVALID_COMMAND : error;
