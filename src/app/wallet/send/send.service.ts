...
export class SendService {
  ...
  constructor(private appService: AppService) { console.log("sendservice active")}

  sendTransaction(input: string, output: string, address: string, amount: number, comment: string, substractfee: boolean, narration: string, ringsize: number, numsignatures: number){
  	this.resetTransactionDetails();
  	// comment is internal, narration is stored on blockchain
  	let rpcCall: string = this.getSendRPCCall(input, output);
  	let anon: boolean = this.isAnon(rpcCall);
  	let params: Array<any> = this.getSendParams(anon, address, amount, comment, substractfee, narration, ringsize, numsignatures);
  ...
  getSendParams(anon: boolean, address: string, amount: number, comment: string, substractfee: boolean, narration: string, ringsize: number, numsignatures: number) {
  	if(anon){
  		//comment-to empty
  ...
  */
  ...
  setTransactionDetails(address, amount){
